...
{
	inform temp;
	FILE* pfout = fopen("contact", "r");//以只读模式打开文本文件
	if(pfout = NULL)
	{
		perror("file::read");
		exit(EXIT_FAILURE);
	}
	while(fread(&temp, sizeof(inform), 1, pfout))//从文件中将联系人信息拷贝至通讯录之中
	{
		Check_capacity(_con);
...
{
	int i = 0;
	FILE* pfin = fopen("contact.txt", "w");//以写入方式打开文件
	if (pfin == NULL)
	{
		perror("file::write");
		exit(EXIT_FAILURE);
	}
	for (i = 0; i < _con->sz; i++)//将联系人信息写入文件
	{
